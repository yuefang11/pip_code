...
def optimization_global(b0_init, b0, score, rho, lamd, alpha, time_limit, count):
    ...
    model.params.LogFile=f'application/model_mip_unconstrained_{count}_alt.log'
    # add the variables
    beta = model.addVars(K, P, vtype=gp.GRB.CONTINUOUS, lb=-1, ub = 1, name="beta")
    ...
    obj = model.objVal
    print(obj, obj + lamd * t.x)
    

    # check the constraint
    try:
        gini = (-(gp.quicksum(z[x_idx, j_idx].x * score_unit[x_idx, j_idx] for x_idx in range(x_card) for j_idx in range(K))) / N + M -
                    (gp.quicksum(score_pair[x_idx1][x_idx2][j_idx1][j_idx2] * (z[x_idx, j_idx].x) * (z[x_idx2, j_idx2].x) for x_idx1 in range(x_card) for x_idx2 in range(x_card) for j_idx1 in range(K) for j_idx2 in range(K)) ) / (N**2)
    ...
    time_used = t1 - t0 
    print(time_used)
    return (b1, obj,  obj+ lamd * t.x, gini, time_used)
...
def optimization(t0_init, relaxed_list, max_iter, frac, ans, b0_init, b0, score, epsilon1, epsilon2, rho, lamd, alpha, tol, cnt = 0, obj = 0, iter = 0):
    ...
    model.setObjective((gp.quicksum(z[i] * score_unit[z_idx[i][0], z_idx[i][1]] 
                    for i in range(len(z_idx))) +  Const_unit)/N - lamd * t, sense = gp.GRB.MAXIMIZE)
    model.addConstrs((beta[j, p] <= s[j, p] for j in range(K) for p in range(P)))
    model.addConstrs((-beta[j, p] <= s[j, p] for j in range(K) for p in range(P)))
    model.addConstr(gp.quicksum(s[j, p] for j in range(K) for p in range(P)) == t )
    # set the constraints
    ...
    # if gamma_var.x == 0:
    obj = model.objVal
    print(obj_init, obj, obj + lamd * t.x)
    print(t.x)
    try:
        gini = (-(gp.quicksum(z[i].x * score_unit[z_idx[i][0], z_idx[i][1]] for i in range(len(z_idx)))  +  Const_unit)/N + M - \
                (gp.quicksum(score_pair[z_idx[i][0]][z_idx[j][0]][z_idx[i][1]][z_idx[j][1]]*(z[i].x)*(z[j].x) for i in range(len(z_idx)) for j in range(len(z_idx)) ) \
    ...
    # gini = (-(gp.quicksum(z[i].x * score_unit[z_idx[i][0], z_idx[i][1]] for i in range(len(z_idx)))  +  Const_unit)/N + M - \
    #             (gp.quicksum(score_pair[z_idx[i][0]][z_idx[j][0]][z_idx[i][1]][z_idx[j][1]]*(z[i].x)*(z[j].x) for i in range(len(z_idx)) for j in range(len(z_idx)) ) \
    #             + 2 * gp.quicksum(score_pair[z_idx[i][0]][right_idx[j][0]][z_idx[i][1]][right_idx[j][1]]*z[i].x*z[j].x for i in range(len(z_idx)) for j in range(len(right_idx))) + Const_pair) / (N**2) ).getValue() \
    #                                     / ((gp.quicksum(z[i].x * score_unit[z_idx[i][0], z_idx[i][1]] for i in range(len(z_idx)))  +  Const_unit)/N).getValue()
    ...
    ans["obj"].append(obj)
    ans["welfare"].append(obj + lamd * t.x)
    # gini = con_trans 
    ...
    if iter >= max_iter or cur_time - t0_init > 3600:
        return (b1, obj, obj+lamd*t.x, gini, time_used, z_idx)
...
# for idx, data in enumerate(folds):
    ...
    with open(f'application/table_unconstrained_{idx}_alt.pkl', 'wb') as fp:
